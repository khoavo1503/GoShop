document.addEventListener('DOMContentLoaded', function() {
    ...
    // <----------------------------Sự kiện chọn màu, size, số lượng--------------------------------->
    const colorButtons = document.querySelectorAll('.product-detail__color button');

    colorButtons.forEach(function(button) {
        button.addEventListener('click', function() {
            // Loại bỏ lớp product-detail--check-color từ tất cả các nút màu
            colorButtons.forEach(function(btn) {
                btn.classList.remove('product-detail--check-color');
            });

            // Thêm lớp product-detail--check-color cho nút màu được click
            this.classList.add('product-detail--check-color');
        });
    });

    const sizeButtons = document.querySelectorAll('.product-detail__size button');

    sizeButtons.forEach(function(button) {
        button.addEventListener('click', function() {
            // Loại bỏ lớp product-detail--check-size từ tất cả các nút kích cỡ
            sizeButtons.forEach(function(btn) {
                btn.classList.remove('product-detail--check-size');
            });

            // Thêm lớp product-detail--check-size cho nút kích cỡ được click
            this.classList.add('product-detail--check-size');
        });
    });
    ...
    // Add click event for each decrement button
    decrementButtons.forEach(function (button) {
        button.addEventListener('click', function () {
            var inputElement = button.parentElement.querySelector('.product-count');
            if (parseInt(inputElement.value) > 1) {
                inputElement.value = parseInt(inputElement.value) - 1;
                updateCountRepeat(button.parentElement);
            }
        });
    });
    ...
    // Add click event for each increment button
    ...
    // Function to update count repeat text
    function updateCountRepeat(parentElement) {
        parentElement.querySelector('.product-count-repeat').textContent = 'Số lượng ' + parentElement.querySelector('.product-count').value;
    }
});
